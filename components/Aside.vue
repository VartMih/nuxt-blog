<template>
  <div class="col-md-4">
    <div class="card my-4">
      <h5 class="card-header aside">Теги</h5>
      <div class="card-body">
    ...
    <div class="card my-4">
      <h5 class="card-header aside">Последние статьи</h5>
      <div class="card-body card-aside" v-for="post in aside" :key="post.id">
        <h5 class="card-title">{{post.h1}}</h5>
        <img :src="post.image" alt="" class="card-img-top">
        <div v-html="post.description" class="truncate"></div>
        <nuxt-link :to="`/posts/${post.slug}`" class="card-link">Далее...</nuxt-link>
        <hr>
      </div>
    </div>
  </div>
